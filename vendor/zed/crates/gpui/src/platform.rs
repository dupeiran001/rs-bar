...
pub trait PlatformWindow: HasWindowHandle + HasDisplayHandle {
    ...
    // Nudge the platform to schedule a draw ASAP.
    //
    // Called by the invalidator when a window transitions from clean to
    // dirty outside of an active frame cycle. Platforms that continuously
    // drive their render loop (the default on macOS / Windows / X11) can
    // leave this as a no-op; the wayland backend overrides it to break
    // out of the quiescent state the conditional re-subscribe patch puts
    // it into.

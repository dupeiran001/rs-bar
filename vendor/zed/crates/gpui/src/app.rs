        ...
        loop {
            ...
            if let Some(effect) = self.pending_effects.pop_front() {
                ...
                if self.pending_effects.is_empty() {
                    // Kick any dirty platform windows whose render loop has
                    // gone quiescent (wayland's conditional re-subscribe;
                    // no-op elsewhere). This makes the wayland backend
                    // schedule a fresh frame callback so the next vsync
                    // picks up the dirty state we just produced.
                    for window_slot in self.windows.values() {
                        if let Some(window) = window_slot.as_deref() {
                            if window.invalidator.is_dirty()
                                && window.invalidator.not_drawing()
                            {
                                window.platform_window.schedule_draw();
                            }
                        }
                    }

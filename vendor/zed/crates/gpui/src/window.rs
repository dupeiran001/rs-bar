...
struct WindowInvalidatorInner {
    ...
    // Invoked when the window transitions from clean to dirty outside of an
    // active draw phase. Used by the wayland backend to break its render
    // loop out of quiescence via PlatformWindow::schedule_draw; other
    // backends leave this unset because they already drive the render loop
    // at vsync regardless of dirty state.
    pub on_clean_to_dirty: Option<Rc<dyn Fn()>>,
}
...
impl WindowInvalidator {
    pub fn new() -> Self {
        WindowInvalidator {
            inner: Rc::new(RefCell::new(WindowInvalidatorInner {
                ...
                on_clean_to_dirty: None,
            })),
        }
    }

    pub fn set_on_clean_to_dirty(&self, callback: Rc<dyn Fn()>) {
        self.inner.borrow_mut().on_clean_to_dirty = Some(callback);
    }
    ...
    pub fn invalidate_view(&self, entity: EntityId, cx: &mut App) -> bool {
        ...
        if inner.draw_phase == DrawPhase::None {
            let was_clean = !inner.dirty;
            inner.dirty = true;
            let on_clean_to_dirty = was_clean.then(|| inner.on_clean_to_dirty.clone()).flatten();
            drop(inner);
            cx.push_effect(Effect::Notify { emitter: entity });
            if let Some(callback) = on_clean_to_dirty {
                callback();
            }
            true
        } else {
    ...
    pub fn set_dirty(&self, dirty: bool) {
        ...
        let was_clean = !inner.dirty;
        inner.dirty = dirty;
        let on_clean_to_dirty = (dirty && was_clean)
            .then(|| inner.on_clean_to_dirty.clone())
            .flatten();
        drop(inner);
        if let Some(callback) = on_clean_to_dirty {
            callback();

...
pub struct WaylandWindowState {
    ...
    // Set to true when draw() actually submits a new scene to the GPU during
    // the current frame cycle. Consumed by frame() to decide whether to
    // re-subscribe to another frame callback: if nothing was drawn we let the
    // render loop go quiescent until something (animation, notify, input)
    // kicks it via schedule_draw().
...
impl WaylandWindowStatePtr {
    ...
    pub fn frame(&self) {
        // Previously we unconditionally re-subscribed to the next frame
        // callback here, which locked the render loop to the compositor's
        // refresh rate (60Hz+165Hz = 225Hz on a dual-monitor setup) even
        // when nothing was changing. For applications whose surface is
        // usually idle (status bars, etc.) this prevents the CPU package
        // from reaching deep C-states.
        //
        // Instead we now re-subscribe only if the request_frame callback
        // actually drew a new scene; an idle frame lets the loop go
        // quiescent, and schedule_draw() is called by the invalidation
        // path to wake it up again when work arrives.
        let force_render = {
            let mut state = self.state.borrow_mut();
            state.resize_throttle = false;
            state.drew_this_frame = false;
            let force = state.force_render_after_recovery;
        ...
        let state = self.state.borrow();
        if state.drew_this_frame {
            state.surface.frame(&state.globals.qh, state.surface.id());
        }
    }

    // Kick the wayland render loop out of its quiescent state.
    //
    // Called by the platform-generic invalidation path (see the
    // PlatformWindow::schedule_draw hook) when something has just marked
    // the window dirty. Requests a one-shot frame callback and commits
    // the current surface state so the compositor schedules us at its
    // next vsync — from which point the normal draw cycle takes over.
    pub fn schedule_draw(&self) {
        let state = self.state.borrow();
        state.surface.frame(&state.globals.qh, state.surface.id());
...
impl PlatformWindow for WaylandWindow {
    ...
    fn schedule_draw(&self) {
        self.0.schedule_draw();
    }
